...
class SpartaKidsRSSGenerator:
    ...
    def create_rss_feed(self, articles):
        ...
        ET.SubElement(channel, 'title').text = 'Sparta Rotterdam Kidsclub nieuws (onofficieel)'
        ET.SubElement(channel, 'link').text = self.site_url
        ET.SubElement(channel, 'description').text = 'Ongeofficieel RSS-nieuwsfeed voor Sparta Rotterdam Kidsclub'
        ET.SubElement(channel, 'language').text = 'nl-NL'
        ET.SubElement(channel, 'lastBuildDate').text = datetime.now().strftime('%a, %d %b %Y %H:%M:%S +0000')
        ET.SubElement(channel, 'generator').text = 'Sparta Rotterdam Kidsclub RSS Generator'
